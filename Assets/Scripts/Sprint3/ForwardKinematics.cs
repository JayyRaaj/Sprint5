...
public Matrix4x4 CalculateForwardKinematics() 
{
    ...
    // Additional end-effector offset beyond the hand link
    Matrix4x4 T_tool = Matrix4x4.Translate(new Vector3(-0.015f, 0, 0.1f)); // Adjust this vector to match the actual tool length

    // Multiply the matrices to get the transformation from base to end-effector including the tool
    Matrix4x4 endEffectorTransform = T_shoulder * T_arm * T_elbow * T_forearm * T_wrist * T_hand * T_tool;
    ...
    void ApplyJointMovements()
{
    for (int i = 0; i < joints.Length; i++)
    {
        ...
        {
            case 0:
                drive.target = shoulder_link; // Shoulder joint
                break;
            case 1:
                drive.target = arm_link; // Arm joint
                break;
            case 2:
                drive.target = elbow_link; // Elbow joint
                break;
            case 3:
                drive.target = forearm_link; // Forearm joint
                break;
            case 4:
                drive.target = wrist_link; // Wrist joint
                break;
            case 5:
                drive.target = hand_link; // Hand joint
                break;
        }
